function  Marker_Band_Power_V6(hn,sn,pn,CLASS,BAND,Fs,Fs_Res,carind)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%            h  = fdesign.bandpass('N,F3dB1,F3dB2', 6, 8,12, Fs);
%            Hd = design(h,'butter');
%            [B, A] = sos2tf(Hd.sosMatrix,Hd.scaleValues);
%            % fvtool(B,A)
%            Fre_Power=filtfilt(B,A,double(Fre_Power));
%            Fre_Power=(abs(hilbert(single(Fre_Power)))).^2;
           Fre_Power_Res=resample(double(Fre_Power),Fs_Res,Fs);
           EMG_Res=resample(EMG,Fs_Res,Fs);
        ...
        for n=1:CLASS_NUM
            ...
            for dl=1:length(index)
                label=[zeros(2*Fs_Res,1);n*ones(5*Fs_Res,1);zeros(3*Fs_Res,1)];
                tindex=index(dl);
                EMG_marker_ind_trial=find(EMG_marker(tindex-Fs*2+1:tindex+Fs*8,1)==1);
                EMG_marker_ind_trial_res=ceil(EMG_marker_ind_trial/(Fs/Fs_Res));
                EMG_marker_Res=zeros(10*Fs_Res,1);
                EMG_marker_Res(EMG_marker_ind_trial_res)=1;
                tindex_res=ceil(tindex/(Fs/Fs_Res));
                New_Data=[Fre_Power_Res(tindex_res-Fs_Res*2+1:tindex_res+Fs_Res*8,:),EMG_Res(tindex_res-Fs_Res*2+1:tindex_res+Fs_Res*8,1),EMG_marker_Res,label];               
                for cc=1:size(New_Data,2)
                    if m==1
                        Power_Data(:,(n-1)*(2*TRIAL_NUM(n))+dl,cc)=New_Data(:,cc);
%                         Phase_Data(:,(n-1)*(2*TRIAL_NUM(n))+dl,cc)=New_Phase(:,cc);
                    else
                        Power_Data(:,(n-1)*(2*TRIAL_NUM(n))+TRIAL_NUM(n)+dl,cc)=New_Data(:,cc);
...
if carind==0
 save(['WITHOUTCAR\',datasetname],'Power_Data','goodchs');
elseif carind==1
 save(['WITHCAR\',datasetname],'Power_Data','goodchs');
elseif carind==2
 save(['WITHCAR_CW_GROUPED\',datasetname],'Power_Data','goodchs');
 elseif carind==3
 save(['LOCALREF\',datasetname],'Power_Data','goodchs');
 elseif carind==4
 save(['WITHCAR_Median\',datasetname],'Power_Data','goodchs');
 elseif carind==5
 save(['WITHCAR_Chn_Spec\',datasetname],'Power_Data','goodchs');
 elseif carind==6
 save(['WITHCAR_Ele_Spec\',datasetname],'Power_Data','goodchs');
elseif carind==7
  save(['BiPolar\',datasetname],'Power_Data','goodchs');
else
end
