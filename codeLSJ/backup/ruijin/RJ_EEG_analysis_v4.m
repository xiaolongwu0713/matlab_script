...
Folder=strcat('D:/lsj/RJ_preprocessing_data/P',num2str(pn),'/analysis');
if ~exist(Folder,'dir')
    mkdir(Folder);
end
cd(Folder);
...
for chn = 1:channelNum
    ...
    for i = 1:TrialNum
        ...
        if triggerInf(triStartTrigger(i)+3, 2) == 4
            NOGOct = NOGOct + 1;
            NOGOtf{NOGOct} = tf(:, TampTriStartTrigger(3):TampTriStartTrigger(3)+6*Fs-1);
        end
