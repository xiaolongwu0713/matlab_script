%function waveletana(data,fs,min_freq,max_freq,num_freq,ploty,norm)
...
fs=256;
%data=epoch(110,:,1);
min_freq=2;max_freq = 128;num_freq=30;
% define wavelet parameters
frequencies = logspace(log10(min_freq),log10(max_freq),num_freq);

wavelet_time = -1:1/fs:1;% srate=256
%s    = logspace(log10(3),log10(10),num_freq)./(2*pi*frequencies);
s=4;
...
baselineidx = [129 206]; % in ms
baseline_power = mean(tf_data(:,baselineidx(1):baselineidx(2)),2);
dbconverted = 10*log10( tf_data ./ repmat(baseline_power,1,n_data) );

figure()
cl=12;
imagesc([1:n_data],[],dbconverted);
ytickskip = 2:4:num_freq;
%set(gca,'ydir','normal','ytick',ytickskip,'yticklabel',round(frequencies(ytickskip)),'clim',[cl*(-1) cl])
